...
#define CHUNK_SIZE_LOG_2 10 // 1024 Chunk size
#define CHUNK_SIZE (1 << CHUNK_SIZE_LOG_2)
#define CHUNK_MASK (CHUNK_SIZE - 1)
STATIC_ASSERT_EQ(CHUNK_SIZE, 1 << CHUNK_SIZE_LOG_2);

#define SMALL_OBJECT_RANDOM_CHUNKS 4
#define MAX_NEW_SLOTS_PER_SO_CHUNK 64 
...
NO_WASLR static struct group_header* get_headerpage(void *ptr) {
  size_t headerpage_ptr = (((uintptr_t)ptr - FIRST_USABLE_BYTE) & ~PAGE_GROUP_SIZE_MASK) + FIRST_USABLE_BYTE;
  return (struct group_header*) headerpage_ptr;
}

NO_WASLR static unsigned get_header_chunk_index(struct group_header *headerpage, void *ptr) {
...
NO_WASLR static void allocate_chunks(struct group_header *page, unsigned idx, enum chunk_kind kind, size_t num_chunks) {
  size_t offset = idx;
  size_t i = 0;
...
NO_WASLR static size_t get_allocation_size(void* ptr) {
    struct group_header *headerpage = get_headerpage(ptr);
...
NO_WASLR static void free_consecutive_chunks(struct group_header *page, unsigned idx, enum chunk_kind kind) {
...
NO_WASLR static int32_t search_headerpage(struct group_header *headerpage, size_t total_hpages, size_t usable_pages_group, size_t num_chunks){
  // we only want to check one byte per usable chunk that follows the header
  size_t max_bytes_to_check = usable_pages_group << PAGE_HEADER_LOG2;
  
  // pick a random offset within the header, which is the header byte for some data chunk
  size_t header_byte_for_chunk = rand_in_range(0, max_bytes_to_check - 1);
  // search for free chunk on the pages represented by the header
...
NO_WASLR static struct search_result find_free_chunks(size_t *found_chunks, size_t num_chunks, size_t num_picks, enum chunk_kind kind, size_t start_at_group) {
  ...
  // number of headers to consider in this search (excludes pages we already searched for this request)
  ...
  while (chunks_left > 0) {
    ...
    // the index of the header within all headers
    size_t selected_group_idx = page_groups[group_array_idx];
  
    // the global page index of the header from the selected group
    size_t global_headerpage_idx = FIRST_USABLE_PAGE_IDX + (selected_group_idx << PAGE_GROUP_LOG2);
    struct group_header *headerpage = (struct group_header *) (global_headerpage_idx << PAGE_SIZE_LOG_2);
    ...
    if (found_chunk_idx == -1) {
      ...
      if (max_index == 0) {
        // we searched all available group headers. Determine which group we search next after allocating more space
        size_t next_group_idx;
...
NO_WASLR static void* allocate_large(size_t size) {
  ...
  size_t found_chunks[1]; // just use an array of size 1 so we do not need extra handling in find_free_chunks
