...
void WebAssemblyFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  if (StackSize){
    ...
    BuildMI(MBB, InsertPt, DL, TII->get(WebAssembly::CALL), SFPtr)
      .addExternalSymbol("ralloc");
      .addReg(SFSizeReg);
  ...
  // we are always using the Base Pointer, but alignment of the SP should only happen if the Base Pointer is actually required
  if (hasBP(MF)) {
...
void WebAssemblyFrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  if (StackSize) {
    RSFPtrReg = FI->getRSFPointerVreg();
    BuildMI(MBB, InsertPt, DL, TII->get(WebAssembly::CALL))
      .addExternalSymbol("rfree");
      .addReg(RSFPtrReg);
  }
}
